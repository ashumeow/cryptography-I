...
def fetchKeyFromMeow(col):
    maxx = 0 ## maximum conflicts
    suspected = '\x00'
    for c in col:
        conflicts = 0
        for cc in col:
    ...
    if maxx < 6: return '\x00'
    return chr( 0x20 ^ ord(suspected) )
    
def fetchMeow(MSGS, i):
    col = ''
    for c in MSGS: col += c[i*2 : (i+1)*2].decode('hex')
    return col
    
def fetchKey(MSGS, l):
    key = ''
    for i in range(l/2):
        col = fetchMeow(MSGS, i)
        k = fetchKeyFromMeow(col)
...
def meowKey(k, in_msg_num, at_pos, right_letter_is):
    def strsetat(s, pos, c):
        return s[0:pos] + c + s[pos+1:]
    
    col = fetchMeow(MSGS, at_pos)
    corrected = ord(col[in_msg_num]) ^ ord(right_letter_is)
